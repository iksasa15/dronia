export function MissionVisionSection() {
  return (
    <section className="section mission-vision" data-reveal aria-label="الرسالة والرؤية">
      <div className="shell mission-grid">
        <article className="mv-card mv-card--accent">
          <p className="section-kicker section-kicker-en">MISSION</p>
          <h2 className="mv-heading">رسالتنا</h2>
          <p>
            تمكين شركائنا من تقديم توثيق جوّي سينمائي متميّز، مع تخفيف أعباء
            التشغيل والإجراءات — من التصاريح إلى تنسيق الطيران يوم الفعالية.
          </p>
        </article>
        <article className="mv-card">
          <p className="section-kicker section-kicker-en">VISION</p>
          <h2 className="mv-heading">رؤيتنا</h2>
          <p>
            الاستفادة من أحدث تقنيات الطائرات المسيّرة بأمان ودقة، وبناء شراكات
            طويلة الأمد مع القاعات والمنسّقين، بما يضمن جودة ثابتة للسوق السعودي.
          </p>
        </article>
      </div>
    </section>
  )
}
